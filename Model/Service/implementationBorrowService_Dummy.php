...
class implementationBorrowService_Dummy implements interfaceBorrowService
{
   ...
   */
   private function __construct()
   {
    ...
    public function setUpdateDueDate($borrowingId, $date)
    {
      $oStatus = $this->getBorrowingStatus($borrowingId);
      if(strcmp($oStatus,"DoesNotExists")!==0)
      {
          $this->_borrowings[$borrowingId-1]['dueDate'] = $date;
      }
    }
    public function getborrowByEnssatPrimaryKey($id)
    {
        $var = null;
        foreach ($this->getBorrowings() as $key)
        {
            if($key['keychainId'] == $id){
                $var = $key;
            }
        }
        return $var;
